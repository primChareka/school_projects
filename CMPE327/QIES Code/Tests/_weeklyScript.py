...
########### FUNCTIONS ##########
def getTestFiles(testFilesPath, day):
    inputFiles = []
    for file in os.listdir(testFilesPath):
        if '%02d'%day+'day_' in file and 'input.txt' in file:
...
def runTerminals(osType, basePath, day):
    ...
    vsf = os.path.join(basePath,'weeklyInputs',vsf)

    #Get a list of the input file names and the expected output file names
    testFilesPath = os.path.join(basePath, 'weeklyInputs')
    inputFiles = getTestFiles(testFilesPath, day)
